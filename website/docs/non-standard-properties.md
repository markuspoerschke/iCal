...
Use the `setPublishedTTL()` method with an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations)-formatted `DateInterval` to set this duration.
...
The `X-WR-CALDESC` property specifies the display description of a calendar.

This property is not part of the iCalendar RFC, but it is used by some calendar clients.

Use the `setCalDescription()` method to set this value.
